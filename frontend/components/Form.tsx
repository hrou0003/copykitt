...
const Form: React.FC<FormProps> = ({ prompt, setPrompt, onSubmit, isLoading }) => {
  ...
  if (isLoading) {
    statusText = <TailSpin color="#FFF" height="1rem" width="1rem" />
  }
    ...
    <>
      ...
      <span>
        <form onSubmit={(e) => e.preventDefault()}>
          <input onKeyPress={onKeyPressHandler} className="p-2 w-full rounded-md focus:outline-teal-400 text-slate-700" type="text" value={prompt} placeholder="coffee" onChange={e => updatePromptValue(e.currentTarget.value)}></input>
        </form>
      </span>
